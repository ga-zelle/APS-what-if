...
def autoISF(sens, target_bg, profile, glucose_status, meal_data, autosens_data, new_parameter, Fcasts, Flows): 
    ...
    if dura70<10 and dura05<10 :
        console_error("gz keep ISF as glucose trend is unclear")
        Flows.append(dict(title="glucose trend is unclear\nskip ISF adaptation", indent='0', adr='isf_93+'))
        Fcasts['emulISF'] = sens
        return sens
        
    maxISFAdaptation = new_parameter['maxISFAdaptation']
    levelISF = 1
    slopeISF = 1
    if dura70>=10 and new_parameter['autoISF_slope']:           # long lasting rise; check first as it more important
        ...
        if slope70 > 0:                                         # catch the rise early
            ...
            if (maxISFAdaptation < prodISF+1) :
                msg_fl = "\nslopeISF capped from "+str(round(prodISF+1,2))+"\nto masISFAdaptation "+str(maxISFAdaptation)
                msg_ce = "; slopeISF capped from "+str(round(prodISF+1,2))+ " to masISFAdaptation "+str(maxISFAdaptation)
            else:
                msg_fl = ""
                msg_ce = ""
    ...
    if dura05>=10 and new_parameter['autoISF_flat']:            # long lasting at level
        ...
        if avg05 > target_bg:                                   # fight the resistance at high levels
            prodISF = new_parameter['prodISF_flat']
            levelISF = new_parameter['liftISF_flat']
            whereto = "more"
            if (maxISFAdaptation < prodISF+1) :
                msg_fl = "\nlevelISF capped from "+str(round(prodISF+1,2))+"\nto masISFAdaptation "+str(maxISFAdaptation)
                msg_ce = "; levelISF capped from "+str(round(prodISF+1,2))+ " to masISFAdaptation "+str(maxISFAdaptation)
            else:
                msg_fl = ""
                msg_ce = ""
            Flows.append(dict(title="avg. above target\nISF("+str(sens)+") not managing it for "+str(dura05)+"m\ngo more aggresive at "+str(round(profile['sens']/levelISF,1))+msg_fl, indent='+1', adr='isf_84+'))
            console_error('gz ISF', sens, 'avg. glucose', avg05, 'for', dura05,'minutes; go', whereto,'aggressive at', round(profile['sens']/levelISF,1), msg_ce)
        else:                                                   # effectively dead as it sometimes backfired
            Flows.append(dict(title="avg. glucose "+str(avg05)+"\nbelow target "+str(target_bg)+"\nskip ISF adaptation", indent='+1', adr='isf_90'))
            console_error("gz ISF unchanged as glucose is below target")
            #    liftISF = min(1.0, (1 + 10.0*dura05/10*pow(avg05-target_bg,2)/target_bg/100))
            #    whereto = "less"
            Fcasts['emulISF'] = sens
            return sens
    sens = round(min(sens, profile['sens'] / min(maxISFAdaptation, max(slopeISF, levelISF))), 1)    # include possible larger autosens effect
    Fcasts['emulISF'] = sens
    return sens
...
def determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctionsDummy, microBolusAllowed, reservoir_data, thisTime, Fcasts, Flows):
    ...
    sens = autoISF(sens, target_bg, profile, glucose_status, meal_data, autosens_data, new_parameter, Fcasts, Flows)
